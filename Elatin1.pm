...
sub Elatin1::fc(@);
sub Elatin1::fc_();
sub Elatin1::ignorecase;
sub Elatin1::classic_character_class;
sub Elatin1::capture;
sub Elatin1::chr(;$);
sub Elatin1::chr_();
sub Elatin1::glob($);
...
# ${Elatin1::eS}        = qr{(?:[^\x09\x0A    \x0C\x0D\x20])};
# ${Elatin1::eS}        = qr{(?:[^\x09\x0A\x0B\x0C\x0D\x20])};
${Elatin1::eS}          = qr{(?:[^\s])};
...
# avoid: Name "Elatin1::foo" used only once: possible typo at here.
${Elatin1::dot}         = ${Elatin1::dot};
${Elatin1::dot_s}       = ${Elatin1::dot_s};
${Elatin1::eD}          = ${Elatin1::eD};
${Elatin1::eS}          = ${Elatin1::eS};
${Elatin1::eW}          = ${Elatin1::eW};
${Elatin1::eH}          = ${Elatin1::eH};
${Elatin1::eV}          = ${Elatin1::eV};
${Elatin1::eR}          = ${Elatin1::eR};
${Elatin1::eN}          = ${Elatin1::eN};
${Elatin1::not_alnum}   = ${Elatin1::not_alnum};
${Elatin1::not_alpha}   = ${Elatin1::not_alpha};
${Elatin1::not_ascii}   = ${Elatin1::not_ascii};
${Elatin1::not_blank}   = ${Elatin1::not_blank};
${Elatin1::not_cntrl}   = ${Elatin1::not_cntrl};
${Elatin1::not_digit}   = ${Elatin1::not_digit};
${Elatin1::not_graph}   = ${Elatin1::not_graph};
${Elatin1::not_lower}   = ${Elatin1::not_lower};
${Elatin1::not_lower_i} = ${Elatin1::not_lower_i};
${Elatin1::not_print}   = ${Elatin1::not_print};
${Elatin1::not_punct}   = ${Elatin1::not_punct};
${Elatin1::not_space}   = ${Elatin1::not_space};
${Elatin1::not_upper}   = ${Elatin1::not_upper};
${Elatin1::not_upper_i} = ${Elatin1::not_upper_i};
${Elatin1::not_word}    = ${Elatin1::not_word};
${Elatin1::not_xdigit}  = ${Elatin1::not_xdigit};
${Elatin1::eb}          = ${Elatin1::eb};
${Elatin1::eB}          = ${Elatin1::eB};
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '${Elatin1::not_lower_i}',
                '[:^upper:]'  => '${Elatin1::not_upper_i}',
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Elatin1::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Elatin1::fc($_[0]) =~ /\A $pattern \z/xms };
...
sub Latin1::getc(;*@) {
    ...
    croak 'Too many arguments for Latin1::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Elatin1::dot_s} \z/oxms) {
...
my $qq_substr  = qr{(?: Latin1::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Elatin1::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin1::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Elatin1::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin1::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Elatin1::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin1::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Elatin1::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Elatin1::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Elatin1::glob';
    }
...
# subroutines of package Elatin1
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Latin1::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Latin1::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Latin1::escape'; }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Elatin1::chop';       }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Latin1::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Latin1::index';       }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Elatin1::index';      }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Latin1::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Latin1::rindex';      }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Elatin1::rindex';     }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin1::lc';         }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin1::lcfirst';    }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin1::uc';         }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin1::ucfirst';    }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin1::fc';         }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin1::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin1::glob';              }
    elsif (/\G \b lc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin1::lc_';               }
    elsif (/\G \b lcfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin1::lcfirst_';          }
    elsif (/\G \b uc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin1::uc_';               }
    elsif (/\G \b ucfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin1::ucfirst_';          }
    elsif (/\G \b fc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin1::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin1::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin1::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Elatin1::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Elatin1::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Elatin1::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Elatin1::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Elatin1::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Elatin1::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Elatin1::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Elatin1::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Elatin1::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Elatin1::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Elatin1::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Elatin1::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Elatin1::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Elatin1::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Elatin1::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Elatin1::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Elatin1::PREMATCH()]}
            ...
            $e_string .= q{Elatin1::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Elatin1::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Elatin1::MATCH()};
            $slash = 'div';
        }
...
# subroutines of package Elatin1
        ...
        elsif ($string =~ /\G \b Latin1::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin1::eval \b                                /oxgc) { $e_string .= 'eval Latin1::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                             /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Elatin1::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin1::index \b                               /oxgc) { $e_string .= 'Latin1::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Elatin1::index';      $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';            $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin1::rindex \b                              /oxgc) { $e_string .= 'Latin1::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Elatin1::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin1::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin1::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin1::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin1::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin1::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin1::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin1::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Elatin1::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Elatin1::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Elatin1::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Elatin1::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Elatin1::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Elatin1::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Elatin1::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Elatin1::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Elatin1::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Elatin1::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Elatin1::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Elatin1::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        ...
        return Elatin1::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Elatin1::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Elatin1::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Elatin1::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Elatin1::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Elatin1::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Elatin1::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Elatin1::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Elatin1::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Elatin1::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Elatin1::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Elatin1::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Elatin1::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Elatin1::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Elatin1::uc($char[$i]) ne Elatin1::fc($char[$i]))) {
            if (CORE::length(Elatin1::fc($char[$i])) == 1) {
                $char[$i] = '['   . Elatin1::uc($char[$i])       . Elatin1::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Elatin1::uc($char[$i]) . '|' . Elatin1::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin1::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Elatin1::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin1::ignorecase(Elatin1::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin1::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Elatin1::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin1::ignorecase(Elatin1::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin1::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Elatin1::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin1::ignorecase(Elatin1::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin1::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Latin1::re_t=%s; while($Latin1::re_t =~ %s){%s local $^W=0; local $Latin1::re_r=%s; %s$Latin1::re_t="$`$Latin1::re_r$'"; pos($Latin1::re_t)=length "$`$Latin1::re_r"; } return $Latin1::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Latin1::re_r=CORE::eval $Latin1::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Latin1::re_r=%s; %s"%s$Latin1::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Latin1::re_n=0; while(%s =~ %s){%s local $^W=0; local $Latin1::re_r=%s; %s%s="$`$Latin1::re_r$'"; pos(%s)=length "$`$Latin1::re_r"; $Latin1::re_n++} return %s$Latin1::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Latin1::re_r=%s; %s%s="%s$Latin1::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Latin1::re_r=%s; %s%s="%s$Latin1::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Latin1::re_r=CORE::eval $Latin1::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Elatin1::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Elatin1::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
This module has run-time routines for use Latin1 software automatically, you
do not have to use.
...
some bugs. Thus, if you happen to find a bug that's in Latin1 software and not
...
=item Split string
  ...
  at all, Elatin1::split returns the original string as a single substring, If it
  ...
  @chars  = Elatin1::split(//,  $word);
  @fields = Elatin1::split(/:/, $line);
  @words  = Elatin1::split(" ", $paragraph);
  @lines  = Elatin1::split(/^/, $buffer);
  ...
  print join(":" => Elatin1::split(/ */, "hi there"));
  ...
  Elatin1::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Elatin1::split(/:/, $_, 3);
  ...
  splits on whitespace just as Elatin1::split with no arguments does. Thus,
  Elatin1::split(" ") can be used to emulate awk's default behavior. In contrast,
  Elatin1::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Elatin1::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Elatin1::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Elatin1::split()) {
          $count{$word}++;
      }
  }

  The inverse of Elatin1::split is join, except that join can only join with the
...
=item Transliteration
  ...
  print Elatin1::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'
...
=item Chop string
  ...
  character chopped. The Elatin1::chop subroutine is used primary to remove the newline
  ...
  since Elatin1::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Elatin1::chop a literal, only a variable. If you Elatin1::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Elatin1::chop(@lines);

  You can Elatin1::chop anything that is an lvalue, including an assignment:

  Elatin1::chop($cwd = `pwd`);
  Elatin1::chop($answer = <STDIN>);

  This is different from:

  $answer = Elatin1::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Elatin1::chop returns the character
  ...
  In the most general case, Elatin1::chop can be expressed using substr:

  $last_code = Elatin1::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Elatin1::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Elatin1::chop;
...
=item Index string
  ...
  $byte_pos = Elatin1::index($string,$substr,$byte_offset);
  $byte_pos = Elatin1::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Elatin1::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Elatin1::rindex($string,$substr,$byte_offset);
  $byte_pos = Elatin1::rindex($string,$substr);

  This subroutine works just like Elatin1::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Elatin1::index).
  ...
  while (($byte_pos = Latin1::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  You can use the Elatin1::fc subroutine for case-insensitive comparisons via Latin1
  software.
...
=item Upper case string
  ...
  in interpolated strings. For titlecase, use Elatin1::ucfirst instead.
...
=item Upper case first character of string
  ...
  $titlecase = Elatin1::ucfirst(substr($word,0,1)) . Elatin1::lc(substr($word,1));
  ...
  $do_not_use = Elatin1::ucfirst(Elatin1::lc($word));
...
=item Fold case string
  ...
  $fc = Elatin1::fc($string);
  $fc = Elatin1::fc_;

  New to Latin1 software, this subroutine returns the full Unicode-like casefold of
  ...
  Elatin1::fc($a) eq Elatin1::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Latin1 software has always used casefolding semantics for
...
=item Make ignore case string
  ...
  @ignorecase = Elatin1::ignorecase(@string);
...
=item Make character
  ...
  that $code in the character set. For example, Elatin1::chr(65) is "A" in either
  ASCII or Latin-1, not Unicode. For the reverse of Elatin1::chr, use Latin1::ord.
...
=item Filename expansion (globbing)
  ...
  The Elatin1::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Elatin1::glob('"*e f*"');
  @spacies = Elatin1::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Elatin1::glob("'*${var}e f*'");
  @spacies = Elatin1::glob(qq("*${var}e f*"));
