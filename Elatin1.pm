...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Elatin1::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Elatin1::open;
    }
}
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Elatin1::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Elatin1::binmode(FILEHANDLE, $disciplines);
  Elatin1::binmode(FILEHANDLE);
  Elatin1::binmode($filehandle, $disciplines);
  Elatin1::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Elatin1::open(FILEHANDLE, $mode, $expr);
  $rc = Elatin1::open(FILEHANDLE, $expr);
  $rc = Elatin1::open(FILEHANDLE);
  $rc = Elatin1::open(my $filehandle, $mode, $expr);
  $rc = Elatin1::open(my $filehandle, $expr);
  $rc = Elatin1::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Elatin1::open($fh, ">logfile")     # $fh is autovivified
  ...
  Elatin1::open my $fh, ">logfile" or die ...
  ...
  Elatin1::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Elatin1::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Elatin1::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Elatin1::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Elatin1::open(FH, "<:crlf:encoding(Latin1)", $path) or die "can't open $path: $!";
