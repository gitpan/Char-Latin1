...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin1::uc($char[$i]) ne Char::Elatin1::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin1::uc($char[$i]) . Char::Elatin1::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin1/Char::Latin1/JA.pm
...
Definitions in Char/Elatin1.pm.
  ...
  @{Char::Elatin1::eV}             qr{(?:[^\x0C\x0A\x0D])}
