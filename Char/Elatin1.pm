...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Elatin1::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Elatin1::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Char::Elatin1::split returns
...
=item Chop string
  ...
  The Char::Elatin1::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Elatin1::index($string,$substr,$position);
  $pos = Char::Elatin1::index($string,$substr);
  ...
  the Char::Elatin1::index function returns -1.

=item Reverse index string

  $pos = Char::Elatin1::rindex($string,$substr,$position);
  $pos = Char::Elatin1::rindex($string,$substr);

  Works just like Char::Elatin1::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Char::Elatin1::chr(65) is "A" in either ASCII or Latin-1, not Unicode,
  and Char::Elatin1::chr(0x82a0) is a Latin-1 HIRAGANA LETTER A. For the reverse of
  Char::Elatin1::chr, use Char::Latin1::ord.
