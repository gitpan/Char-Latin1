...
my $function_ord;         # ord()   to ord()   or Latin1::ord()
my $function_ord_;        # ord     to ord     or Latin1::ord_
my $function_reverse;     # reverse to reverse or Latin1::reverse
my $function_getc;        # getc    to getc    or Latin1::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Latin1::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Latin1::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Latin1::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Latin1::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Latin1::re_t=%s; while($Latin1::re_t =~ %s){%s local $^W=0; local $Latin1::re_r=%s; %s$Latin1::re_t="$`$Latin1::re_r$'"; pos($Latin1::re_t)=length "$`$Latin1::re_r"; } return $Latin1::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Latin1::re_r=eval $Latin1::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Latin1::re_r=%s; %s"%s$Latin1::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Latin1::re_n=0; while(%s =~ %s){%s local $^W=0; local $Latin1::re_r=%s; %s%s="$`$Latin1::re_r$'"; pos(%s)=length "$`$Latin1::re_r"; $Latin1::re_n++} return %s$Latin1::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Latin1::re_r=%s; %s%s="%s$Latin1::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Latin1::re_r=%s; %s%s="%s$Latin1::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Latin1::re_r=eval $Latin1::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Elatin1.pm
works well.
...
=head1 Calling 'Elatin1::ignorecase()' (Latin1.pm provides)

Latin1.pm applies calling 'Elatin1::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Latin1.pm provides)

Latin1.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Latin1.pm and Elatin1.pm provide)
...
=head1 Un-Escaping bytes::* Subroutines (Latin1.pm provides)

Latin1.pm removes 'bytes::' at head of subroutine name.
