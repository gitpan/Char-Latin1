...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Elatin1::uc($char[$i]) ne Elatin1::lc($char[$i]))) {
            $char[$i] = '[' . Elatin1::uc($char[$i]) . Elatin1::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Latin1/Latin1/JA.pm
